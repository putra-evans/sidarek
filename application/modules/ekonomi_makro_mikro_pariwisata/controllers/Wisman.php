...
class Wisman extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_wisman' => 'mwis'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('WISMAN (Wisatawan Mancanegara)', '#');

        $this->session_info['page_name'] = "Pariwisata - WISMAN (Wisatawan Mancanegara)";
        $this->session_info['bangsa'] = $this->getBangsa();
        $this->template->build('form_wisman/list', $this->session_info);
    }

    public function getBangsa()
    {

        $query = $this->db->get("ma_pariwisata_bangsa");
        $data = [];
        $data[''] = '-- Pilih Kebangsaan --';
        foreach ($query->result() as $key => $value) {
            $data[$value->id_bangsa] = $value->nama_bangsa;
        }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mwis->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mwis->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mwis->insert_data();
            }
    ...
    public function export_to_excel()
    {
        ...
        $template  = 'repository/template/pariwisata.xls';
        $objPHPExcel = $objReader->load($template);
        //get data
        $bangsa         = escape($this->input->get('bangsa', TRUE));
        $tahun          = escape($this->input->get('tahun', TRUE));

        $datapariwisata   = $this->mwis->getDataList($bangsa, $tahun);
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', 'DAFTAR PARIWISATA');
        //set data hospital
        $noRow = 0;
        $baseRow = 6;
        if (count($datapariwisata) > 0) {
            foreach ($datapariwisata as $key => $dh) {
                $noRow++;
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $row, $dh['nama_bangsa']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $row, $dh['bulan_pariwisata']);
