...
class Umkmb extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_umkmb' => 'mumkmb'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Data UMK dan UMB', '#');

        $this->session_info['page_name'] = "Data UMKMB";
        $this->session_info['regency'] = $this->getRegency();

        $this->template->build('form_umkmb/list', $this->session_info);
    }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mumkmb->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mumkmb->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mumkmb->insert_data();
            }
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->mumkmb->update_data();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
    ...
    public function export_to_excel()
    {
        ...
        $template  = 'repository/template/umkmb.xls';
        $objPHPExcel = $objReader->load($template);
        //get data
        $tahun       = escape($this->input->get('tahun', TRUE));
        $kabkota        = escape($this->input->get('kabkota', TRUE));

        $dataUMKMB   = $this->mumkmb->getDataListumkmbReport($tahun, $kabkota);
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', 'DATA UMKMB');
        //set data hospital
        $noRow = 0;
        $baseRow = 7;
        if (count($dataUMKMB) > 0) {
            foreach ($dataUMKMB as $key => $dh) {
                $noRow++;
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $row, $dh['umk']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('E' . $row, $dh['umb']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $row, $dh['total_umkmb']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $row, $dh['tk_umk']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $row, $dh['tk_umb']);
