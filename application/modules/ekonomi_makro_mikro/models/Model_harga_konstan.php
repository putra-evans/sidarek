...
class Model_harga_konstan extends CI_Model
{
	...
	public function validasiDataValue($role)
	{
		...
		$this->form_validation->set_rules('harga_konstan', 'Harga Konstan', 'required');
	...
	public function _get_datatables($param)
	{
		...
		$this->db->where('a.jenis_pdrb', 'harga konstan');
		...
		if (isset($post['triwulan_filter']) and $post['triwulan_filter'] != '') {
			$this->db->where('a.triwulan', $post['triwulan_filter']);
		}
	...
	public function insert_data()
	{
		...
		try {
			...
			$data = array(
				...
				'harga_pdrb'					=> $this->input->post('harga_konstan'),
			...
			$result['success'] = 'YEAH';
			$result['status'] = true;
			$result['message'] = 'Data Harga Konstan Berhasil Disimpan';
		} catch (\Exception $e) {
	...
	public function update_data()
	{
		...
		try {
			...
			if ($passed) {
				...
				$result['success'] = 'YEAH';
				$result['status'] = true;
				$result['message'] = 'Data Harga Konstan Berhasil Diubah';
			}
		} catch (\Exception $e) {
			$result['info'] = $e->getMessage();
		}
