<script type="text/javascript">
    $(function() {
        ...
        mainTbl = $('#mainTbl').dataTable({
            ...
            ordering: false,
            ajax: {
                url: site + "pembangunan_manusia/pembangunan_manusia/listview/",
                type: "POST",
                data: function(d) {
        ...
        // Form Reset
        function formReset() {
            $('#formEntry').attr('action', site + 'pembangunan_manusia/pembangunan_manusia/create');
            $('#errEntry').html('');
            $('.help-block').text('');
            $('.required').removeClass('has-error');
            $('form#formEntry').trigger('reset');
            $('.lblPass').text('*');
            $('#komponen').select2('val', '');
        }
        ...
        $(document).on('click', '.btnEdit', function(e) {
            ...
            e.stopPropagation();
            formReset();
            $('#formEntry').attr('action', site + 'pembangunan_manusia/pembangunan_manusia/update');

            $('#id_pembangunan').val(data.id_pembangunan);
            $('#komponen').select2('val', data.komponen);
            $('#add_monday_date').val(data.tahun);
                ...
                buttons: {
                    ...
                    "main": {
                        ...
                        callback: function(response) {
                            if (response) {
                                $.ajax({
                                    url: site + 'pembangunan_manusia/pembangunan_manusia/delete',
                                    type: "POST",
                                    data: postData,
                                    dataType: "json",
                                }).done(function(data) {
        ...
        $(document).on('click', '#printExcel', function(e) {
            ...
            url = site + 'pembangunan_manusia/pembangunan_manusia/export-to-excel?tahun=' + tahun;
