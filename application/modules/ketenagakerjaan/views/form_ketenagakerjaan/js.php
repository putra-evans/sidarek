<script type="text/javascript">
    $(function() {
        ...
        mainTbl = $('#mainTbl').dataTable({
            ...
            ordering: false,
            ajax: {
                url: site + "ketenagakerjaan/ketenagakerjaan/listview/",
                type: "POST",
                data: function(d) {
        ...
        // Form Reset
        function formReset() {
            $('#formEntry').attr('action', site + 'ketenagakerjaan/ketenagakerjaan/create');
            $('#errEntry').html('');
            $('.help-block').text('');
        ...
        $(document).on('click', '.btnEdit', function(e) {
            ...
            e.stopPropagation();
            formReset();
            $('#formEntry').attr('action', site + 'ketenagakerjaan/ketenagakerjaan/update');

            $('#id_kerja').val(data.id_kerja);
            $('#stt').select2('val', data.id_status);

            $('#add_monday_date').val(data.bulan_kerja);
            $('#jumlah').val(data.jumlah_kerja);
                ...
                buttons: {
                    ...
                    "main": {
                        ...
                        callback: function(response) {
                            if (response) {
                                $.ajax({
                                    url: site + 'ketenagakerjaan/ketenagakerjaan/delete',
                                    type: "POST",
                                    data: postData,
                                    dataType: "json",
                                }).done(function(data) {
        ...
        $(document).on('click', '#printExcel', function(e) {
            ...
            url = site + 'ketenagakerjaan/ketenagakerjaan/export-to-excel?status=' + status + '&tahun=' + tahun;
