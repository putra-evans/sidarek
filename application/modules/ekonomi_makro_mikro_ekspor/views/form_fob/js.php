<script type="text/javascript">
    $(function() {
        ...
        mainTbl = $('#mainTbl').dataTable({
            ...
            ordering: false,
            ajax: {
                url: site + "ekonomi_makro_mikro_ekspor/fob/listview/",
                type: "POST",
                data: function(d) {
        ...
        // Form Reset
        function formReset() {
            $('#formEntry').attr('action', site + 'ekonomi_makro_mikro_ekspor/fob/create');
            $('#errEntry').html('');
            $('.help-block').text('');
            $('.required').removeClass('has-error');
            $('form#formEntry').trigger('reset');
            $('.lblPass').text('*');
            $('#id_sektor').select2('val', '');
        }
        ...
        $(document).on('click', '.btnEdit', function(e) {
            ...
            e.stopPropagation();
            formReset();
            $('#formEntry').attr('action', site + 'ekonomi_makro_mikro_ekspor/fob/update');

            $('#id_ekspor').val(data.id_ekspor);
            $('#id_sektor').select2('val', data.id_sektor_ekspor);

            $('#add_monday_date').val(data.bulan_ekspor);
            $('#nilaifob').val(data.nilai_fob);
                ...
                buttons: {
                    ...
                    "main": {
                        ...
                        callback: function(response) {
                            if (response) {
                                $.ajax({
                                    url: site + 'ekonomi_makro_mikro_ekspor/fob/delete',
                                    type: "POST",
                                    data: postData,
                                    dataType: "json",
                                }).done(function(data) {
        ...
        $(document).on('click', '#printExcel', function(e) {
            let sektor = $('#formFilter').find('select[name="sektor"]').val();
            let tahun = $('#formFilter').find('input[name="monday_date1"]').val();

            // //(sektor)
            // let id_tipe = $('#formFilter').find('select[name="id_tipe"]').val();
            url = site + 'ekonomi_makro_mikro_ekspor/fob/export-to-excel?sektor=' + sektor + '&tahun=' + tahun;
