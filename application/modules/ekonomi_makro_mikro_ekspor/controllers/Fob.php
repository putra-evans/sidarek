...
class Fob extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_FOB' => 'mfob'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Nilai FOB', '#');

        $this->session_info['page_name'] = "Ekspor - Nilai FOB";
        $this->session_info['sektor'] = $this->getSektor();
        $this->template->build('form_fob/list', $this->session_info);
    }

    public function getSektor()
    {

        $query = $this->db->get("ma_sektor_ekspor");
        $data = [];
        $data[''] = '-- Pilih Sektor --';
        foreach ($query->result() as $key => $value) {
            $data[$value->id_sektor_ekspor] = $value->sektor_ekspor;
        }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mfob->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mfob->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mfob->insert_data();
            }
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                $data = $this->mfob->delete_data();
                if ($data['message'] == 'SUCCESS') {
                    $result = array('status' => 1, 'message' => 'Data FOB Ekspor berhasil dihapus...', 'csrf' => $this->csrf);
                } else {
                    $result = array('status' => 0, 'message' => 'Proses delete data FOB Ekspor gagal, silahkan periksa kembali data yang akan dihapus...', 'csrf' => $this->csrf);
                }
            } else {
    ...
    public function export_to_excel()
    {
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', 'DAFTAR NILAI FOB EKSPOR');
        //set data hospital
        $noRow = 0;
        $baseRow = 6;
        if (count($fob) > 0) {
            foreach ($fob as $key => $dh) {
                $noRow++;
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $row, $dh['sektor_ekspor']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $row, $dh['bulan_ekspor']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $row, $dh['nilai_fob']);
