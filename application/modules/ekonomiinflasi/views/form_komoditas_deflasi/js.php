<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    $(document).on('click', '#printExcel', function(e) {
      let jenis = $('#formFilter').find('select[name="jenis_komoditas"]').val();
      let bulan = $('#formFilter').find('input[name="monday_date"]').val();

      url = site + 'ekonomiinflasi/komoditas_deflasi/export-to-excel?jenis=' + jenis + '&bulan=' + bulan;
  ...
  mainTbl = $('#mainTbl').dataTable({
    ...
    ordering: false,
    ajax: {
      url: site + "ekonomiinflasi/komoditas_deflasi/listview",
      type: "POST",
      data: function(d) {
  ...
  // Form Reset
  function formReset() {
    $('#formEntry').attr('action', site + 'ekonomiinflasi/komoditas_deflasi/create');
    $('#errEntry').html('');
    $('.help-block').text('');
  ...
  $(document).on('click', '.btnEdit', function(e) {
    ...
    e.stopPropagation();
    formReset();
    $('#formEntry').attr('action', site + 'ekonomiinflasi/komoditas_deflasi/update');
    $('#id_kom_inflasi').val(data.id_kom_inflasi);
    $('#komoditas').select2('val', data.id_komoditas_penyumbang);
    $('#inflasi').val(data.inflasi_deflasi);
    $('#andil').val(data.andil);
    $('#add_monday_date').val(data.bulan);
  ...
  $(document).on('click', '.btnDelete', function(e) {
    ...
    var idkominflasi = $(this).data('id');
    var postData = {
      'id_kom_inflasi': idkominflasi,
      ...
      buttons: {
        ...
        "main": {
          ...
          callback: function(response) {
            if (response) {
              $.ajax({
                url: site + 'ekonomiinflasi/komoditas_deflasi/delete',
                type: "POST",
                data: postData,
                dataType: "json",
              }).done(function(data) {
