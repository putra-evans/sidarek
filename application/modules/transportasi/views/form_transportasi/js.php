<script type="text/javascript">
    $(function() {
        ...
        mainTbl = $('#mainTbl').dataTable({
            ...
            ordering: false,
            ajax: {
                url: site + "transportasi/transportasi/listview/",
                type: "POST",
                data: function(d) {
        ...
        // Form Reset
        function formReset() {
            $('#formEntry').attr('action', site + 'transportasi/transportasi/create');
            $('#errEntry').html('');
            $('.help-block').text('');
            $('.required').removeClass('has-error');
            $('form#formEntry').trigger('reset');
            $('.lblPass').text('*');
            $('#id_bangsa').select2('val', '');
        }
        ...
        $(document).on('click', '.btnEdit', function(e) {
            ...
            e.stopPropagation();
            formReset();
            $('#formEntry').attr('action', site + 'transportasi/transportasi/update');

            $('#id_transportasi').val(data.id_transportasi);
            $('#jenis_penerbangan').select2('val', data.jenis_penerbangan);

            $('#add_monday_date').val(data.bulan_transportasi);
            $('#jumlah_berangkat').val(data.jumlah_berangkat);
            $('#jumlah_datang').val(data.jumlah_datang);
                ...
                buttons: {
                    ...
                    "main": {
                        ...
                        callback: function(response) {
                            if (response) {
                                $.ajax({
                                    url: site + 'transportasi/transportasi/delete',
                                    type: "POST",
                                    data: postData,
                                    dataType: "json",
                                }).done(function(data) {
        ...
        $(document).on('click', '#printExcel', function(e) {
            let jenis = $('#formFilter').find('select[name="jenis"]').val();
            let tahun = $('#formFilter').find('input[name="monday_date"]').val();

            //(tahun)
            // let id_tipe = $('#formFilter').find('select[name="id_tipe"]').val();
            url = site + 'transportasi/transportasi/export-to-excel?jenis=' + jenis + '&tahun=' + tahun;
