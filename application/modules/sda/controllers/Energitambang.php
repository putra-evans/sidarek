...
class Energitambang extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_energitambang' => 'mtambang'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Energi dan Pertambangan', '#');

        $this->session_info['page_name']        = "Energi dan Pertambangan";
        $this->session_info['list_sektor']      = $this->mtambang->getListSektor();
        $this->session_info['list_komoditi']    = $this->mtambang->getListKomoditi();
        $this->template->build('form_energitambang/list', $this->session_info);
    }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mtambang->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mtambang->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mtambang->insert_data_produksi();
            }
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                $data = $this->mtambang->delete_data_produksi();
                if ($data['message'] == 'SUCCESS') {
                    $result = array('status' => 1, 'message' => 'Data Energi dan Pertambangan Berhasil Dihapus...', 'csrf' => $this->csrf);
                } else {
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->mtambang->update_data_produksi();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
    ...
    public function export_to_excel()
    {
        ...
        $dataProduksi   = $this->mtambang->getDataListProduksiReport($sektor, $komoditi);
