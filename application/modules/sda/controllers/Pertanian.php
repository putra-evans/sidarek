...
class Pertanian extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_pertanian' => 'mp'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Pertanian', '#');

        $this->session_info['page_name'] = "Pertanian";
        $this->session_info['list_sektor']    = $this->mp->getListSektor();
        $this->session_info['list_komoditi']    = $this->mp->getListKomoditi();
        $this->template->build('form_pertanian/list', $this->session_info);
    }
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                ...
                if ($data['message'] == 'SUCCESS') {
                    $result = array('status' => 1, 'message' => 'Data Pertanian Berhasil Dihapus...', 'csrf' => $this->csrf);
                } else {
