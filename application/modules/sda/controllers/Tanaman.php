...
class Tanaman extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_tanaman' => 'mt'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Tanaman Pangan dan Holtikultura', '#');

        $this->session_info['page_name'] = "Tanaman Pangan dan Holtikultura";
        $this->session_info['list_sektor']    = $this->mt->getListSektor();
        $this->session_info['list_komoditi']    = $this->mt->getListKomoditi();
        $this->template->build('form_tanaman/list', $this->session_info);
    }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mt->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mt->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mt->insert_data_produksi();
            }
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                $data = $this->mt->delete_data_produksi();
                if ($data['message'] == 'SUCCESS') {
                    $result = array('status' => 1, 'message' => 'Data Tanaman Pangan dan Holtikultura Berhasil Dihapus...', 'csrf' => $this->csrf);
                } else {
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->mt->update_data_produksi();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
    ...
    public function export_to_excel()
    {
        ...
        $dataProduksi   = $this->mt->getDataListProduksiReport($sektor, $komoditi);
