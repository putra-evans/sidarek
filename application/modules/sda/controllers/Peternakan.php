...
class Peternakan extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_peternakan' => 'mpeternakan'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Peternakan', '#');

        $this->session_info['page_name'] = "Peternakan";
        $this->session_info['list_sektor']    = $this->mpeternakan->getListSektor();
        $this->session_info['list_komoditi']    = $this->mpeternakan->getListKomoditi();
        $this->template->build('form_peternakan/list', $this->session_info);
    }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mpeternakan->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mpeternakan->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mpeternakan->insert_data_produksi();
            }
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                $data = $this->mpeternakan->delete_data_produksi();
                if ($data['message'] == 'SUCCESS') {
                    $result = array('status' => 1, 'message' => 'Data Peternakan Berhasil Dihapus...', 'csrf' => $this->csrf);
                } else {
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->mpeternakan->update_data_produksi();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
    ...
    public function export_to_excel()
    {
        ...
        $dataProduksi   = $this->mpeternakan->getDataListProduksiReport($sektor, $komoditi);
