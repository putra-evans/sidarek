...
class Kehutanan extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_kehutanan' => 'mk'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Kehutanan', '#');

        $this->session_info['page_name'] = "Kehutanan";
        $this->session_info['list_sektor']    = $this->mk->getListSektor();
        $this->session_info['list_komoditi']    = $this->mk->getListKomoditi();
        $this->template->build('form_kehutanan/list', $this->session_info);
    }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mk->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mk->validasiDataValue('new') == false) {
                ...
                $result = $this->mk->insert_data_produksi();
            }
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                ...
                if ($data['message'] == 'SUCCESS') {
                    $result = array('status' => 1, 'message' => 'Data Kehutanan Berhasil Dihapus...', 'csrf' => $this->csrf);
                } else {
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->mk->update_data_produksi();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
    ...
    public function export_to_excel()
    {
        ...
        $dataProduksi   = $this->mk->getDataListProduksiReport($sektor, $komoditi);
        ...
        header('Expires: Mon, 26 Jul 1997 05:00:00 GMk'); // Date in the past
        header('Last-Modified: ' . gmdate('D, d M Y H:i:s') . ' GMk'); // always modified
