...
class Produksi extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_produksi' => 'mpsi'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Kebijakan Produksi Sumber Daya Alam', '#');

        $this->session_info['page_name'] = "Produksi Sumber Daya Alam";
        $this->session_info['list_sektor']    = $this->mpsi->getListSektor();
        $this->template->build('form_produksi/list', $this->session_info);
    }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mpsi->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mpsi->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mpsi->insert_data_produksi();
            }
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                $data = $this->mpsi->delete_data_produksi();
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->mpsi->update_data_produksi();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
    ...
    public function export_to_excel()
    {
        ...
        $dataProduksi   = $this->mpsi->getDataListProduksiReport($sektor, $komoditi);
