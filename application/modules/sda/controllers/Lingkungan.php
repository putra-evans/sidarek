...
class Lingkungan extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_lingkungan' => 'ml'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Lingkungan Hidup', '#');

        $this->session_info['page_name'] = "Lingkungan Hidup";
        $this->session_info['list_sektor']    = $this->ml->getListSektor();
        $this->session_info['list_komoditi']    = $this->ml->getListKomoditi();
        $this->template->build('form_lingkungan/list', $this->session_info);
    }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->ml->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->ml->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->ml->insert_data_produksi();
            }
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                $data = $this->ml->delete_data_produksi();
                if ($data['message'] == 'SUCCESS') {
                    $result = array('status' => 1, 'message' => 'Data Lingkungan Hidup Berhasil Dihapus...', 'csrf' => $this->csrf);
                } else {
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->ml->update_data_produksi();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
    ...
    public function export_to_excel()
    {
        ...
        $dataProduksi   = $this->ml->getDataListProduksiReport($sektor, $komoditi);
