...
class Model_produksi extends CI_Model
{
	...
	public function _get_datatables_query($param)
	{
		...
		if (isset($post['id_sektor_filter']) and $post['id_sektor_filter'] != '') {
			$this->db->where('b.id_sektor', $post['id_sektor_filter']);
		}
	...
	public function insert_data_produksi()
	{
		...
		try {
			...
			$result['success'] = 'YEAH';
			$result['status'] = true;
			$result['message'] = 'Data Produksi SDA Berhasil Disimpan';
		} catch (\Exception $e) {
