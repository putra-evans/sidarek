...
class Model_lingkungan extends CI_Model
{
	...
	public function insert_data_produksi()
	{
		...
		try {
			...
			$result['success'] = 'YEAH';
			$result['status'] = true;
			$result['message'] = 'Data Lingkungan Hidup Berhasil Disimpan';
		} catch (\Exception $e) {
	...
	public function update_data_produksi()
	{
		...
		try {
			...
			if ($produk) {
				...
				$result['success'] = 'YEAH';
				$result['status'] = true;
				$result['message'] = 'Data Lingkungan Hidup Berhasil diupdate';
			}
		} catch (\Exception $e) {
			$result['info'] = $e->getMessage();
		}
