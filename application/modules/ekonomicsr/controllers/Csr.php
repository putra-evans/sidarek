...
class Csr extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_csr' => 'mcsr'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->session_info['page_name'] = "Data Ekonomi - Saluran Dana CSR";
        $this->session_info['perusahaan'] = $this->getPerusahaan();
        $this->template->build('form_csr/list', $this->session_info);
    }

    public function getPerusahaan()
    {

        $query = $this->db->get("ref_perusahaan");
        $data = [];
        $data[''] = '-- Pilih Perusahaan --';
        foreach ($query->result() as $key => $value) {
            $data[$value->id_perusahaan] = $value->nama;
        }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mcsr->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mcsr->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mcsr->insert_data();
            }
    ...
    public function export_to_excel()
    {
        ...
        $template  = 'repository/template/csr.xls';
        $objPHPExcel = $objReader->load($template);
        //get data
        $perusahaan       = escape($this->input->get('perusahaan', TRUE));

        $dataCSR   = $this->mcsr->getDataListCSRReport($perusahaan);
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', 'DAFTAR CSR');
        //set data hospital
        $noRow = 0;
        $baseRow = 6;
        if (count($dataCSR) > 0) {
            foreach ($dataCSR as $key => $dh) {
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $row, $dh['nama_perusahaan']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C' . $row, $dh['tahun']);
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('D' . $row, $dh['alokasi_dana']);
