<script type="text/javascript">
  $(function() {
    ...
    mainTbl = $('#mainTbl').dataTable({
      ...
      ordering: false,
      ajax: {
        url: site + "ekonomicsr/csr/listview",
        type: "POST",
        data: function(d) {
      ...
      columns: [{
        ...
        },
        {
          ...
          render: function(data, type, row, meta) {
            return numberWithCommas(row.alokasi_dana);
          }
        },
        {
          data: "realisasi_kemitraan",
          className: "text-right",
          render: function(data, type, row, meta) {
            return numberWithCommas(row.realisasi_kemitraan);
          }
        },
        {
          data: "realisasi_bina_lingkungan",
          className: "text-right",
          render: function(data, type, row, meta) {
            return numberWithCommas(row.realisasi_bina_lingkungan);
          }
        },
        {
          ...
          className: "text-right",
          render: function(data, type, row, meta) {
            return numberWithCommas(row.jumlah_realisasi);
          }
        },
        {
          data: "action",
    ...
    // Form Reset
    function formReset() {
      $('#formEntry').attr('action', site + 'ekonomicsr/csr/create');
      $('#errEntry').html('');
      $('.help-block').text('');
    ...
    $(document).on('click', '.btnEdit', function(e) {
      ...
      e.stopPropagation();
      formReset();
      $('#formEntry').attr('action', site + 'ekonomicsr/csr/update');

      $('#id_csr').val(data.id_csr);
      $('#tahun').select2('val', data.tahun);
      $('#id_perusahaan').select2('val', data.id_perusahaan);


      $('#alokasi_dana').val(data.alokasi_dana);
      $('#realisasi_kemitraan').val(data.realisasi_kemitraan);
      $('#realisasi_bina_lingkungan').val(data.realisasi_bina_lingkungan);
      $('#jumlah_realisasi').val(data.jumlah_realisasi);
        ...
        buttons: {
          ...
          "main": {
            ...
            callback: function(response) {
              if (response) {
                $.ajax({
                  url: site + 'ekonomicsr/csr/delete',
                  type: "POST",
                  data: postData,
                  dataType: "json",
                }).done(function(data) {
    ...
    $(document).on('click', '#printExcel', function(e) {
      let id_perusahaan_filter = $('#formFilter').find('select[name="id_perusahaan_filter"]').val();

      // //(id_perusahaan_filter)
      // let tahun = $('#formFilter').find('select[name="tahun"]').val();
      // let id_tipe = $('#formFilter').find('select[name="id_tipe"]').val();
      url = site + 'ekonomicsr/csr/export-to-excel?perusahaan=' + id_perusahaan_filter;
