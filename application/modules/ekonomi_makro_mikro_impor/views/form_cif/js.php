<script type="text/javascript">
    $(function() {
        ...
        mainTbl = $('#mainTbl').dataTable({
            ...
            ordering: false,
            ajax: {
                url: site + "ekonomi_makro_mikro_impor/cif/listview/",
                type: "POST",
                data: function(d) {
        ...
        // Form Reset
        function formReset() {
            $('#formEntry').attr('action', site + 'ekonomi_makro_mikro_impor/cif/create');
            $('#errEntry').html('');
            $('.help-block').text('');
            $('.required').removeClass('has-error');
            $('form#formEntry').trigger('reset');
            $('.lblPass').text('*');
            $('#id_golongan').select2('val', '');
        }
        ...
        $(document).on('click', '.btnEdit', function(e) {
            ...
            e.stopPropagation();
            formReset();
            $('#formEntry').attr('action', site + 'ekonomi_makro_mikro_impor/cif/update');

            $('#id_impor').val(data.id_impor);
            $('#id_golongan').select2('val', data.id_golongan);

            $('#add_monday_date').val(data.bulan_impor);
            $('#nilaicif').val(data.nilai_cif);
                ...
                buttons: {
                    ...
                    "main": {
                        ...
                        callback: function(response) {
                            if (response) {
                                $.ajax({
                                    url: site + 'ekonomi_makro_mikro_impor/cif/delete',
                                    type: "POST",
                                    data: postData,
                                    dataType: "json",
                                }).done(function(data) {
        ...
        $(document).on('click', '#printExcel', function(e) {
            let golongan = $('#formFilter').find('select[name="golongan"]').val();
            let tahun = $('#formFilter').find('input[name="monday_date"]').val();

            // //(sektor)
            // let id_tipe = $('#formFilter').find('select[name="id_tipe"]').val();
            url = site + 'ekonomi_makro_mikro_impor/cif/export-to-excel?golongan=' + golongan + '&tahun=' + tahun;
