...
class KreditSektor extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_sektor' => 'msektor'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Kredit per sektor', '#');

        $this->session_info['page_name'] = "Kredit per sektor";
        $this->session_info['sektor_ekonomi'] = $this->getSektor();
    ...
    public function getSektor()
    {
        ...
        $query = $this->db->get("ref_sektor_ekonomi");
        $data = [];
        $data[''] = '-- Semua Sektor --';
        foreach ($query->result() as $key => $value) {
            $data[$value->id_sektor_ekonomi] = $value->nama;
        }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->msektor->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->msektor->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->msektor->insert_data();
            }
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->msektor->update_data();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
    ...
    public function export_to_excel()
    {
        ...
        $template  = 'repository/template/kredit_sektor.xls';
        $objPHPExcel = $objReader->load($template);
        //get data
        $tahun       = escape($this->input->get('tahun', TRUE));
        $sektor      = escape($this->input->get('sektor', TRUE));

        $dataWilayah   = $this->msektor->getDataListkreditsektorReport($tahun, $sektor);
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', 'DAFTAR KREDIT PER SEKTOR');
        //set data hospital
        $noRow = 0;
        $baseRow = 7;
        if (count($dataWilayah) > 0) {
            foreach ($dataWilayah as $key => $dh) {
                $noRow++;
                ...
                $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B' . $row, $dh['sektor_name']);
