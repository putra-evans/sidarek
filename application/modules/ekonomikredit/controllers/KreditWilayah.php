...
class KreditWilayah extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_wilayah' => 'mwilayah'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->breadcrumb->add('Kredit per Wilayah', '#');

        $this->session_info['page_name'] = "Kredit per Wilayah";
        $this->session_info['regency'] = $this->getRegency();

        $this->template->build('form_wilayah/list', $this->session_info);
    }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $resultDT = $this->mwilayah->process_datatables($param);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mwilayah->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mwilayah->insert_data();
            }
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                $data = $this->mwilayah->delete_data();
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->mwilayah->update_data();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
    ...
    public function export_to_excel()
    {
        ...
        $template  = 'repository/template/kredit_wilayah.xls';
        $objPHPExcel = $objReader->load($template);
        //get data
        $tahun        = escape($this->input->get('tahun', TRUE));
        $kabkota      = escape($this->input->get('kabkota', TRUE));

        $dataWilayah   = $this->mwilayah->getDataListkreditwilayahReport($tahun, $kabkota);
        ...
        $objPHPExcel->setActiveSheetIndex(0)->setCellValue('A2', 'DAFTAR KREDIT PER WILAYAH');
        //set data hospital
        $noRow = 0;
        $baseRow = 7;
