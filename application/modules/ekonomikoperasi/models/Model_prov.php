...
class Model_prov extends CI_Model
{
	...
	public function process_datatables($param, $tahun)
	{
		...
		foreach ($dataProduk as $r) {
			$data[] = array(
				'index' 					=> $index,
				'id_peragaan_prov' 			=> $r['id_peragaan_prov'],
				'id_peragaan_koperasi' 		=> $r['id_peragaan_koperasi'],
				'kategori_koperasi' 		=> $r['kategori_koperasi'],
				'tahun' 					=> $r['tahun'],
				...
				'action' 					=> '<button type="button" class="btn btn-xs btn-orange btn-status-warning btnEdit" data-id="' . $this->encryption->encrypt($r['id_peragaan_prov']) . '" title="Edit data"><i class="fa fa-pencil"></i> </button>
											<button type="button" class="btn btn-xs btn-danger btn-status-danger btnDelete" data-id="' . $this->encryption->encrypt($r['id_peragaan_prov']) . '" title="Delete data"><i class="fa fa-times"></i> </button>'
		...
		$result = array(
			...
			"recordsTotal" => $this->db->count_all_results('ta_peragaan_prov'),
			"recordsFiltered" => $this->db->count_all_results('ta_peragaan_prov'),
			"data" => $data
		);
	...
	public function _get_datatables($param, $tahun)
	{
		...
		// Query Filter
		$this->db->select('
							peragaan.id_peragaan_prov,
							peragaan.id_peragaan_koperasi,
							peragaan.kategori_koperasi,
							peragaan.tahun,
							...
							peragaan.total_snu
		');
		$this->db->from('ta_peragaan_prov peragaan');
	...
	public function insert_data()
	{
		...
		try {
			...
			$this->db->insert('ta_peragaan_prov', $data);
	...
	public function update_data()
	{
		...
		try {
			...
			$idPassed = $this->input->post('id_peragaan_prov');
			$passed   = $this->db->where('id_peragaan_prov', $idPassed);
			...
			if ($passed) {
				...
				$this->db->update('ta_peragaan_prov', $data);
	...
	{
		$item     = $this->encryption->decrypt($this->input->post('id_peragaan_prov', true));

		/*query delete*/
		$this->db->where('id_peragaan_prov', $item);
		$this->db->delete('ta_peragaan_prov');
