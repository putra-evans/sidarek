...
class Komoditas extends SLP_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array('model_komoditas' => 'mkomoditas'));
        $this->csrf = array(
    ...
    public function index()
    {
        ...
        $this->template->build('form_komoditas/list', $this->session_info);
    }
    ...
    public function listview()
    {
        ...
        if (!$this->input->is_ajax_request() || empty($session)) {
            ...
            $id_komoditas_for_jenis = $this->input->post('id_komoditas_for_jenis', true);
            $id_kategori_for_jenis = $this->input->post('id_kategori_for_jenis', true);


            $resultDT = $this->mkomoditas->process_datatables($param, $id_komoditas_for_jenis, $id_kategori_for_jenis);
    ...
    public function create()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if ($this->mkomoditas->validasiDataValue('new') == false) {
                $result = array('status' => 0, 'message' => $this->form_validation->error_array());
            } else {
                $result = $this->mkomoditas->insert_data();
            }
    ...
    public function delete()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($session)) {
                $data = $this->mkomoditas->delete_data();
    ...
    public function update()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $result = $this->mkomoditas->update_data();
            $result['csrf'] = $this->csrf;
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
